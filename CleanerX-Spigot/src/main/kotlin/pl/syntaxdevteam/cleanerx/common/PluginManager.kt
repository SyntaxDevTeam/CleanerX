...
class PluginManager(private val plugin: CleanerX) {
    ...
    init {
        ...
        if (highestPriorityPlugin == plugin.description.name) {
            val syntaxDevTeamPlugins = loadedPlugins.filter { it.first != plugin.description.name }
    ...
    private fun fetchLoadedPlugins(): List<Pair<String, String>> {
        ...
        for (plugin in plugin.server.pluginManager.plugins) {
            if (plugin.description.authors.contains("SyntaxDevTeam")) {
                plugins.add(Pair(plugin.description.name, plugin.description.version))
            }
        }
        return plugins
    }
